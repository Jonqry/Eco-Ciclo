...
import { Search, Map as MapIcon, X, Filter, Layers } from 'lucide-react';
...
export default function PontosDeColetaPage() {
  ...
  return (
    <div className="relative w-full h-screen bg-white font-sans overflow-hidden">
      ...
      {/* MAPA EM TELA CHEIA (FUNDO COM NOMES DE RUAS CLAROS) */}
      <div className="absolute inset-0 z-0">
        ...
        >
          <TileLayer
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" // Mapa detalhado com ruas e nomes
                ...
                <div className="p-2 min-w-[150px]">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {ponto.tiposResiduosAceitos.split(',' ).map((t, i) => (
                      <span key={i} className="text-[8px] bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded font-black uppercase">
                        {t.trim()}
                      </span>
                    ))}
                  </div>
      ...
      {/* BARRA LATERAL FLUTUANTE (ESTILO PREMIUM) */}
      <aside className="absolute top-4 left-4 bottom-4 w-full max-w-[420px] z-[1000] flex flex-col pointer-events-none">
        <div className="bg-white/95 backdrop-blur-sm rounded-[32px] shadow-2xl shadow-emerald-900/20 flex flex-col h-full pointer-events-auto overflow-hidden border border-white/50">
          ...
          <div className="p-8 pb-6">
            ...
            <form onSubmit={(e) => { e.preventDefault(); fetchPontos(searchTerm); }} className="relative group">
              <input
                ...
                className="w-full pl-12 pr-12 py-4 bg-slate-100 border-2 border-transparent rounded-2xl text-sm font-bold text-slate-700 placeholder:text-slate-400 focus:bg-white focus:border-emerald-500 focus:ring-0 transition-all shadow-inner"
                value={searchTerm}
                onChange={(e) => {
          ...
          {/* Lista de Cards com Scroll Refinado */}
          ...
          <div className="p-6 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between">
            <button className="flex items-center gap-2 text-[11px] font-black text-emerald-600 uppercase tracking-widest hover:text-emerald-700 transition-colors">
